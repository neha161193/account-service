...
public class PaymentService {
    ...
    public Response processPayment(PaymentRequestDTO paymentRequest) throws JsonProcessingException {
        Account account = accountRepository.findByCustomerIdAndAccountNo(paymentRequest.getToAccount().getCustomerId(), 
        paymentRequest.getToAccount().getAccountNo());

        if(account == null){
            return Response.status(Response.Status.NOT_FOUND).entity("Account Doesn't Exist").build();
        }
        ...
        if (paymentRequest.getAmount().compareTo(BigDecimal.ZERO) > 0) {
            LOG.infof("Processing payment: Account=%s, Amount=%.2f", paymentRequest.getToAccount().getAccountNo(), paymentRequest.getAmount());
            ...
            LOG.warnf("Payment failed: Account=%s, Amount=%.2f", paymentRequest.getToAccount().getAccountNo(), paymentRequest.getAmount());
            transaction.setResponseTimestamp(LocalDateTime.now());
